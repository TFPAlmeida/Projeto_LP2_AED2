...
public class BinomialMinPQ<Key> implements Iterable<Key> {
	...
	public void insert(Key key) {
		...
		x.key = key;
		x.order = 0;
		algs4.BinomialMinPQ<Key> H = new algs4.BinomialMinPQ<Key>(); //The Comparator oh the H heap is not used
		H.head = x;
	...
	public Key delMin() {
		...
		if (min.child != null) {
			...
			x.sibling = prevx;
			algs4.BinomialMinPQ<Key> H = new algs4.BinomialMinPQ<Key>();
			H.head = x;
			head = union(H).head;
		}
		return min.key;
	 ...
	 */
	public algs4.BinomialMinPQ<Key> union(algs4.BinomialMinPQ<Key> heap) {
	...
	private class MyIterator implements Iterator<Key> {
		algs4.BinomialMinPQ<Key> data;
		
		//Constructor clones recursively the elements in the queue
		//It takes linear time
		public MyIterator() {
			data = new algs4.BinomialMinPQ<Key>(comp);
