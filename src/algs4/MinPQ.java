...
public class MinPQ<Key> implements Iterable<Key> {
    ...
    ***************************************************************************/
    ...
    private class HeapIterator implements Iterator<Key> {
        // create a new pq
        private algs4.MinPQ<Key> copy;

        // add all items to copy of heap
        // takes linear time since already in heap order so no keys move
        public HeapIterator() {
            if (comparator == null) copy = new algs4.MinPQ<Key>(size());
            else                    copy = new algs4.MinPQ<Key>(size(), comparator);
            for (int i = 1; i <= n; i++)
                copy.insert(pq[i]);
        }
    ...
    public static void main(String[] args) {
        algs4.MinPQ<String> pq = new algs4.MinPQ<String>();
        while (!StdIn.isEmpty()) {
